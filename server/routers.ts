...
- When explaining hand rankings or rules, use both English and Chinese terms for clarity`;
  ...
  marketing: router({
    ...
    broadcastToUsers: adminProcedure.input(z.object({
      ...
      content: z.string().min(1),
      imageUrl: z.string().optional(),
      buttons: z.array(z.object({ text: z.string(), url: z.string().optional(), callback_data: z.string().optional(), type: z.string().optional(), row: z.number().optional() })).optional(),
    })).mutation(async ({ input, ctx }) => {
      const { createBroadcastTask, executeBroadcast } = await import("./marketing");
      const adminId = (ctx.user?.id) ?? 0;
    ...
    couponPush: adminProcedure.input(z.object({
      ...
      if (input.mode === "group") {
        ...
        const taskId = await createBroadcastTask({
          ...
          content: msgText,
          imageUrl: null,
          buttons: [{ text: "🎁 领取优惠券", callback_data: `redeem_coupon_${coupon.code}`, type: "callback", row: 0 }],
          targetType: "all",
          targetUserIds: null,
          scheduledAt: null,
          createdBy: 0,
    ...
    checkinPush: adminProcedure.input(z.object({
      ...
      if (input.mode === "group") {
        ...
        const taskId = await createBroadcastTask({
          ...
          content: msgText,
          imageUrl: null,
          buttons: [{ text: "✅ 立即签到", callback_data: "checkin", type: "callback", row: 0 }],
          targetType: "all",
          targetUserIds: null,
          scheduledAt: null,
          createdBy: 0,
    ...
    eventPush: adminProcedure.input(z.object({
      ...
      if (input.mode === "group") {
        ...
        const taskId = await createBroadcastTask({
          ...
          content: msgText,
          imageUrl: null,
          buttons: [{ text: "🎉 参与活动", callback_data: `event_${event.id}`, type: "callback", row: 0 }],
          targetType: "all",
          targetUserIds: null,
          scheduledAt: null,
          createdBy: 0,
    ...
    fissionPush: adminProcedure.input(z.object({
      ...
      if (input.mode === "group") {
        ...
        const taskId = await createBroadcastTask({
          ...
          content: msgText,
          imageUrl: null,
          buttons: [{ text: "🚀 获取邀请链接", callback_data: `fission_${campaign.linkCode}`, type: "callback", row: 0 }],
          targetType: "all",
          targetUserIds: null,
          scheduledAt: null,
          createdBy: 0,
