...
## 2026-06-17 管理后台用户列表布局优化
- [x] 添加用户类型筛选：全部/真实用户/机器人
- [x] 用户列表改为两行布局，固定对齐
- [x] 第一行：用户名+ID + IP/地区 + 设备 + 余额 + 风控等级
- [x] 第二行：注册时间 + 最后登录 + 奖金 + 在线状态
- [x] 统计卡片增加：总充值、总提现、今日充值、今日提现
- [x] 默认筛选“真实用户”，机器人用灰色头像+🤖图标区分

## 2026-06-17 用户数据清理 + 语音播报 + 财务统计
...
## 2026-06-18 结算界面重新设计
- [x] 结算弹窗移到公牌下方中间位置(top:58%)，只显示赢家名+金额+牌型
- [x] 去掉其他玩家对比列表、彩纸动画、奖杯图标、边池信息
- [x] 去掉赢家头像金色边框动画(animate-winner-glow)
- [x] 去掉赢钱数字标签(+$xx.xx) 和 5个金币图标
- [x] 赢家牌加黄色边框高亮(border-2 border-gold + glow shadow)，和公牌一起展示

## 2026-06-18 大厅布局修复 + 1000机器人生成
...
- [x] DB: 创建 red_packets 表（标题、总金额、总数量、已领数量、已领金额、类型random/fixed、领取条件JSON、状态、过期时间、图片）
- [x] DB: 创建 red_packet_claims 表（红包ID、用户ID、金额、领取时间）
- [x] 后端: 创建红包 API（admin）
- [x] 后端: 红包列表/详情 API
- [x] 后端: 用户领取红包 API（含条件验证、随机金额分配）
- [x] 前端admin: 红包管理面板（创建、列表、详情、领取记录）
- [x] 前端user: 红包领取页面（在Mini App内）
- [x] 群发集成: 红包消息可通过群发发送到TG频道/用户（通过群发按钮配置 webapp 链接实现）

## Bug Fix: 群发按钮缺少链接类型选择

- [x] ButtonEditor 增加按钮类型选择（url链接 / webapp / callback）
- [x] 按钮编辑器UI优化：增加列标题标签，明确每列含义
- [x] 检查NotificationsPanel创建对话框是否缺少按钮/图片配置

## Feature: 营销系统资金明细 + 活动链接

- [x] 裂变活动: 显示活动链接（可见+复制按钮）
- [x] 裂变活动: 资金发放明细列表（总体统计栏：总活动/总点击/总注册/总发放奖励）
- [x] 优惠券: 总发放金额汇总统计卡片
- [x] 优惠券: 使用明细记录（统计栏：总使用次数/总发放金额/近7日领取）
- [x] 签到奖励: 签到统计汇总（今日签到人数、总发放金额、累计签到次数、独立用户数）
- [x] 签到奖励: 签到明细记录（统计栏展示）
- [x] 邀请奖励: 首充优惠统计栏（总领取/总发放奖励/总充值金额）
...
## 管理后台 HD 钱包统计面板
- [x] 后端：adminHdWallet.stats 接口（已分配地址数、待确认充值笔数、已到账总额、归集总额）
- [x] 后端：adminHdWallet.consolidations 列表接口（分页、状态筛选）
- [x] 后端：adminHdWallet.deposits 列表接口（分页、状态筛选）
...
## Bug Fix: 场次标签 + 充值HD模式
- [x] 场次分类标签优化：去掉英文，改为简洁中文（如“初级场 0.02”）
- [x] 充值页面：管理后台选了HD模式后，客户端自动显示## Bug Fix: 提现 + 钱包地址保存 + Tab标签
- [x] 修复提现报错：添加 withdrawAddress/withdrawChain 字段到 users 表，修复查询报错
- [x] 提现钱包地址保存到用户信息，下次自动填充（提现成功后自动保存，再次进入自动填充）
- [x] 场次Tab筛选标签优化（去掉括号格式，改为圆形badge显示数量，更简洁）化（"全部场次(5) 初级场(2)"等标签显示优化）

## Bug Fix: HD充值地址派生错误
- [x] 修复HD地址派生返回主钱包地址的bug：派生索引从1开始跳过index=0，并增加与主钱包地址对比检测
- [x] 确认地址持久化保存到deposit_addresses表，下次直接读取不重新派生
- [x] 清除错误的index=0记录（与主钱包地址相同）

## Bug Fix: 提现SQL错误 + 钱包余额显示
- [x] 修复提现SQL错误：列名 frozen_balance 改为 frozenBalance（与数据库实际列名一致）
- [x] 钱包分开显示可提金额（绿色）和待解冻金额（黄色）

## PC端布局 + 启动页动画
- [x] PC端：改为居中手机容器布局（max-w-[430px]限制，两侧深色背景+扑克花色装饰+金色光晕+边框阴影）
- [x] 启动页：德州扑克主题动画启动页（飞入扑克牌、旋转筹码、公共牌发牌动画、绿色牌桌光晕、加密连接提示）
- [x] 启动页对所有用户显示（不仅TG），强制显示2.5秒，使用fixed全屏覆盖

## 启动页优化 + PC布局修改
- [x] 启动页加载时间延长（从2.5秒改为4秒）
- [x] 中间五张公共牌改为同花顺 A K Q J 10（同一花色♠），牌面放大
- [x] PC端布局：添加TG Mini App风格顶部标题栏（左侧Vera Poker，右侧三个点+关闭按钮），不全屏，带圆角边框

## 启动页增强 + 游戏桌面修复
- [x] 启动页加载时间再增加1.5秒（总共5.5秒）
- [x] 启动页添加淡出过渡动画（500ms淡出），切换到大厅更自然
- [x] 同花顺牌在动画结束时整齐排列好（卡片先散开再回归对齐）
- [x] 启动页添加发牌音效（上传到webdev-storage）
...
- [x] 问题3：私人房免买入弹窗，直接从用户余额扣（全部余额，上限maxBuyIn）

## Bot决策引擎深度优化v4
- [x] 实现牌面强度分析（自己牌+公牌匹配度：对子/两对/顺子/同花/葫芦等细化评估）
- [x] 实现对手行为推断系统（下注轮数/金额/模式→推断对手牌力范围）
...
## Bot多语言用户名
- [x] 实现14种语言的真实用户名库（英/中/日/韩/俄/阿拉伯/泰/越/印尼/葡/西/法/德/土耳其）
- [x] 按比例随机分配bot用户名到各语言
...
## 用户管理：在线玩家筛选
- [x] 后端增加在线玩家查询接口（基于room_players表status=active）
- [x] 管理后台用户列表增加"在线玩家"筛选按钮/Tab
- [x] 导出功能同样支持在线玩家筛选

## 用户管理UI优化
- [x] 用户列表每个用户增加在线状态绿点标志
- [x] 在线玩家筛选下分为"真人在线"和"机器人在线"两个分区展示

## 比赛场TG富文本分享
- [x] 实现比赛场分享按钮，生成TG富文本消息（图片+文字+按钮）
- [x] 分享链接带邀请码，新用户通过链接注册成为下线
- [x] 分享到TG用户或群组

## 私人房输入优化
- [x] 修复输入房间号时键盘遮挡问题（scrollIntoView + padding-bottom优化）
...
## Bug Fix: 赢家播报与结算界面不一致
- [x] 结算界面primaryWinner排序从按金额改为按牌型强度，与语音播报逻辑统一

## Bug Fix: 锦标赛按钮和连续发牌
- [x] 锦标赛桌不显示"开始下一局"按钮（前端已有条件判断，补充了waiting状态下的tournamentInfo返回）
- [x] 锦标赛开始后自动发第一手牌（不需要玩家手动点击）
- [x] 防止连续发牌：startNewHand加锁防并发 + auto-start检查当前手牌状态

## Bug Fix: 锦标赛重连被误判为重复入座
- [x] 后端：锦标赛桌玩家已入座时返回成功（RECONNECTED）而非报错
- [x] 前端：排除锦标赛桌的私人房自动加入逻辑，避免触发重复入座错误

## Feature: 锦标赛完整结算界面
- [x] 锦标赛结束时显示完整排名列表（头像、昵称、排名、奖金）
- [x] 顶部显示个人排名和奖金
- [x] 前三名显示奖杯/奖牌图标
- [x] 当前用户高亮标记
