...
export default function Profile() {
  ...
  return (
    <div className="min-h-screen bg-deep-space pb-20">
      ...
      {/* Avatar & Name + Account Info (Compact) */}
        ...
        <div className="glass rounded-2xl p-4">
                ...
                {editingNickname ? (
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      className="glass rounded-lg px-2 py-0.5 text-sm outline-none focus:ring-1 focus:ring-gold w-28"
                      placeholder={t("profile.nicknamePlaceholder")}
                      autoFocus
                    />
                    <button onClick={() => updateMutation.mutate({ nickname: newNickname })} className="p-0.5 rounded bg-gold/10 text-gold"><Check className="w-3.5 h-3.5" /></button>
                    <button onClick={() => setEditingNickname(false)} className="p-0.5 rounded bg-red-500/10 text-red-400"><X className="w-3.5 h-3.5" /></button>
                  </div>
                ) : (
                  <>
                    <h2 className="text-base font-bold truncate">{displayName}</h2>
                    <button onClick={() => { setNewNickname(profile.nickname || profile.name || ""); setEditingNickname(true); }} className="p-0.5 text-muted-foreground"><Edit2 className="w-3 h-3" /></button>
                  </>
                )}
              </div>
              <div className="flex items-center gap-2 mt-0.5 text-[11px] text-muted-foreground">
                <span>ID: #{profile.id}</span>
                {profile.tgId && <span>· TG {t("profile.tgBound")}</span>}
                {profile.inviteCode && <span>· {t("profile.inviteCode")}: {profile.inviteCode}</span>}
              </div>
            </div>
          </div>
          {/* Balance & Stats row */}
          <div className="flex items-center justify-around mt-3 pt-3 border-t border-border/40">
            <div className="text-center">
              <p className="text-base font-bold text-gold">{formatBalance(profile.balance)}</p>
              <p className="text-[10px] text-muted-foreground">{t("profile.balance")}</p>
            </div>
            <div className="w-px h-7 bg-border/50" />
            <div className="text-center">
              <p className="text-base font-bold text-truth-blue">{profile.totalGamesPlayed}</p>
              <p className="text-[10px] text-muted-foreground">{t("profile.totalGames")}</p>
            </div>
            <div className="w-px h-7 bg-border/50" />
            <div className="text-center">
              <p className="text-base font-bold text-muted-foreground">{profile.agentLevel === "agent" ? t("profile.agentLevelAgent") : t("profile.agentLevelUser")}</p>
              <p className="text-[10px] text-muted-foreground">{t("profile.agentLevel")}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pb-2">
        <div className="glass rounded-2xl px-4 py-2.5">
          <div className="flex items-center justify-between text-[11px]">
            <span className="text-muted-foreground">{t("profile.registeredAt")}: {formatDate(profile.createdAt)}</span>
            <span className="text-muted-foreground">{t("profile.lastLogin")}: {formatDate(profile.lastSignedIn)}</span>
          </div>
        </div>
      </div>
